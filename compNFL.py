...
def run_multiple_ga(features_df, target, n_runs=10, selection_threshold=0.25):
    ...
    for i in range(n_runs):
        ...
        print(f"Selected features in run {i+1}: {selected_features}")
...
def main():
    # Load dataset
    input_file = 'FootballData/nfl_team_stats_2002-2023.csv'
    ...
    # Set target variable
    target = df['score_home']
    df = df.drop(columns=['score_home'])
    
    # Filter features that contain 'home'
    home_features = [col for col in df.columns if 'home' in col.lower()]
    df = df[home_features]
    
    # Normalize data
    normalized_df = normalize_columns(df, df.columns.tolist())
    ...
    for idx, col in enumerate(normalized_df.columns):
        features = calculate_feature(X0, idx, l, d)
        features_dict[col] = features
    
    features_cart = pd.DataFrame(features_dict)
    target_cart = target.iloc[l:].reset_index(drop=True)
    
    # Run multiple GA iterations
    final_features = run_multiple_ga(features_cart, target_cart, n_runs=1, selection_threshold=0.3)
    ...
    y_train = target_cart[:split_idx]
    y_test = target_cart[split_idx:]
