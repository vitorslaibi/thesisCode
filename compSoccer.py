...
def main():
    # Load dataset
    input_file = 'SoccerData/2020-2021.csv'
    ...
    # Set target variable
    target = df['FTHG']
    df = df.drop(columns=['FTHG'])    
    h_features = [col for col in df.columns if 'H' in col]
    df = df[h_features]
    ...
    final_features = run_multiple_ga(features_cart, target_cart, n_runs=10, selection_threshold=0.3)
